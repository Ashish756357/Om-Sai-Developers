...
const whatsappUrl = `https://wa.me/919876543210?text=${encodeURIComponent("Hello Om Sai Developers, I'd like to know more about Serene Palms Township in Dapoli.")}`;

function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="mb-4 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.22em] text-[#D97757]"><span className="h-px w-8 bg-[#D97757]" />{children}</p>;
}
...
function LeadForm({ compact = false, onSuccess }: { compact?: boolean; onSuccess?: () => void }) {
  ...
  return (
    <form onSubmit={submit} className={compact ? 'flex flex-col gap-3 lg:flex-row' : 'space-y-4'}>
      ...
      {compact && <select required name="size" defaultValue="" className="field"><option value="" disabled>Preferred plot size</option><option>1,500 - 2,000 sq. ft.</option><option>2,000 - 3,000 sq. ft.</option><option>3,000+ sq. ft.</option></select>}
    ...
    <main className="overflow-hidden bg-[#FAF7F2] text-[#16352a]">
      <header className="glass-panel fixed inset-x-0 top-0 z-40 border-b border-white/15 text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          <a href="#top" className="flex items-center gap-3 text-sm font-semibold tracking-tight"><span className="relative grid h-9 w-9 place-items-center overflow-hidden rounded-full bg-[#D97757] text-[#FAF7F2]"><LiquidMetalLogo /><Leaf className="relative z-10" size={18} /></span><span>Om Sai Developers <em className="block text-[10px] font-normal tracking-[0.18em] text-[#d8e5d4]">SERENE PALMS</em></span></a>
      ...
      <section id="top" className="relative isolate flex min-h-[760px] items-end overflow-hidden bg-[#1b4332] pb-10 pt-36 text-white lg:min-h-[810px] lg:pb-16">
        ...
        <div className="relative mx-auto w-full max-w-7xl px-5 lg:px-8"><div className="max-w-4xl"><motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7 }}><p className="mb-5 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-[#f4bd9e]"><MapPin size={14} /> Village Nargoli, Dapoli</p><h1 className="max-w-4xl font-serif text-5xl leading-[.98] tracking-[-.03em] md:text-7xl lg:text-[5.4rem]">Your own corner of the <span className="text-[#f4bd9e]">Konkan.</span></h1><p className="mt-6 max-w-2xl text-base leading-7 text-[#e7eee8] md:text-lg">Own Your Coastal Villa Plot in the Mini Mahabaleshwar of Konkan. Collector-sanctioned NA plots and bespoke holiday villas, made for unhurried days.</p><div className="builder-proof mt-7"><span><strong>25+</strong> years building trust</span><span><strong>100%</strong> clear paperwork</span><span><strong>On-site</strong> support</span></div></motion.div>
          <div className="glass-panel mt-9 max-w-5xl p-4 md:p-5"><p className="mb-3 text-xs font-bold uppercase tracking-[.16em] text-[#f4bd9e]">Receive the master plan + brochure</p><LeadForm compact onSuccess={() => setModal('visit')} /><p className="mt-3 text-xs text-[#d5e2d6]">Free pickup from Khed Railway Station for your site visit.</p></div>
        </div></div>
      </section>

      <section className="border-b border-[#d9d8ce] bg-[#f0ede5]"><div className="mx-auto grid max-w-7xl divide-y divide-[#d9d8ce] px-5 py-1 md:grid-cols-2 md:divide-x md:divide-y-0 lg:grid-cols-4 lg:px-8">{[['100%', 'Collector-sanctioned NA'], ['01', 'Separate 7/12 extract per plot'], ['TP', 'Town Planning sanctioned'], ['Bank', 'Loan assistance available']].map(([big, text]) => <div key={text} className="flex items-center gap-4 py-5 md:px-6 first:md:pl-0"><span className="font-serif text-3xl text-[#D97757]">{big}</span><span className="text-xs font-semibold uppercase leading-5 tracking-[.08em] text-[#496052]">{text}</span></div>)}</div></section>
      ...
      <section id="amenities" className="bg-[#1b4332] py-20 text-[#FAF7F2] lg:py-28"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="max-w-2xl"><SectionLabel>Amenities & infrastructure</SectionLabel><h2 className="heading text-[#FAF7F2]">Everything you need, <span>nothing you don&apos;t.</span></h2></div><div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{[[Trees, 'Recreation & wellness', 'Modern clubhouse, landscaped garden, yoga deck and a playful corner for little explorers.'], [Droplets, 'Sustainability', 'Rainwater harvesting, solar street lights, Alphonso mango, cashew and coconut plantation.'], [Zap, 'Utilities', 'Storm-water drainage, dedicated transformer and overhead storage tanks.'], [Flower2, 'Services', 'On-site caretaker, horticulture assistance and turnkey villa construction support.']].map(([Icon, title, copy]) => <div key={title as string} className="border-t border-white/20 pt-5"><Icon size={22} className="mb-10 text-[#f4bd9e]" /><h3 className="text-lg font-bold">{title as string}</h3><p className="mt-3 text-sm leading-6 text-[#c8d8ca]">{copy as string}</p></div>)}</div></div></section>

      <section id="master-plan" className="section-space"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><SectionLabel>Master plan</SectionLabel><h2 className="heading">Find your <span>place here.</span></h2><p className="body-copy mt-4">Tap an available plot to see its details.</p></div><div className="flex gap-4 text-xs font-semibold text-[#66766a]"><span><i className="status-dot bg-[#508e60]" /> Available</span><span><i className="status-dot bg-[#d7a847]" /> Booked</span><span><i className="status-dot bg-[#ba5b4e]" /> Sold</span></div></div><div className="mt-10 grid gap-8 lg:grid-cols-[1.4fr_.6fr]"><div className="relative overflow-hidden border border-[#cfd4ca] bg-[#e4e9df] p-5 md:p-10"><div className="absolute inset-x-10 top-1/2 h-px bg-[#c2cdbf]" /><div className="absolute inset-y-10 left-1/2 w-px bg-[#c2cdbf]" /><div className="relative grid grid-cols-3 gap-3 md:gap-5">{plots.map((plot) => <button key={plot.id} onClick={() => openPlot(plot)} disabled={plot.status !== 'Available'} className={`plot plot-${plot.status.toLowerCase()}`}><span>Plot</span><strong>#{plot.id}</strong><small>{plot.size.replace(' sq. ft.', '')}</small></button>)}</div><div className="mt-8 flex items-center justify-center gap-3 text-xs text-[#617064]"><MapPin size={14} /> Internal avenue road <span className="mx-2 h-px w-12 bg-[#879888]" /> Main entry</div></div><div className="border border-[#d9d8ce] bg-[#f0ede5] p-7"><MapPin className="text-[#D97757]" size={24} /><h3 className="mt-12 text-xl font-bold">A garden township on the hill.</h3><p className="mt-3 text-sm leading-6 text-[#68766b]">Gentle contours, generous green pockets and clear east-west orientation make each address feel open and private.</p><button onClick={() => setModal('visit')} className="button-primary mt-8 w-full">Ask about availability <ArrowUpRight size={16} /></button></div></div></div></section>

      <section className="border-y border-[#d9d8ce] bg-[#f0ede5] py-20 lg:py-24"><div className="mx-auto max-w-7xl px-5 lg:px-8"><SectionLabel>Configurations</SectionLabel><div className="grid gap-5 md:grid-cols-2"><div className="flex flex-col justify-between border border-[#d2d4c9] bg-[#FAF7F2] p-7 md:p-10"><div><span className="text-xs font-bold uppercase tracking-[.18em] text-[#D97757]">Option A</span><h2 className="mt-5 font-serif text-4xl">Raw NA plots</h2><p className="mt-4 max-w-md text-sm leading-6 text-[#68766b]">Choose your canvas, from 1,500 sq. ft. to 5,000+ sq. ft., with the freedom to design at your own rhythm.</p></div><a href="#contact" className="mt-12 flex items-center gap-2 text-sm font-bold text-[#1b4332]">Get current availability <ArrowUpRight size={16} /></a></div><div className="flex flex-col justify-between bg-[#D97757] p-7 text-[#fff8f0] md:p-10"><div><span className="text-xs font-bold uppercase tracking-[.18em] text-[#ffe1cc]">Option B</span><h2 className="mt-5 font-serif text-4xl">Turnkey villas</h2><p className="mt-4 max-w-md text-sm leading-6 text-[#fff0e3]">1, 2 and 3 BHK Konkan-style homes with red laterite stone, sloping roofs and the ease of a considered build.</p></div><a href="#contact" className="mt-12 flex items-center gap-2 text-sm font-bold">Talk to our villa team <ArrowUpRight size={16} /></a></div></div></div></section>
